  ...
  useEffect(() => {
    ...
    api.get<Inscription[]>(`/inscriptions/user/${session.userId}`)
      .then(async (inscriptions) => {
        const eventIds = [...new Set(inscriptions.map((insc) => insc.eventId))];
        const eventEntries = await Promise.all(
          eventIds.map(async (eventId) => {
            try {
              const event = await api.get<EventSummary>(`/events/${eventId}`);
              return [eventId, event.name] as const;
            } catch (error) {
              console.error(error);
              return [eventId, `Evento #${eventId}`] as const;
            }
          })
        );
        ...
        const eventNameMap = new Map(eventEntries);
        setReservations(
          inscriptions.map((inscription) => ({
            inscription,
            eventName: eventNameMap.get(inscription.eventId) || `Evento #${inscription.eventId}`
          }))
        );
      })
    ...
    <div className="page-wrapper container" style={{ paddingBottom: '4rem' }}>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          {[1, 2, 3].map((item) => <InscriptionSkeleton key={item} />)}
        </div>
      ) : reservations.length > 0 ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gap: '1.5rem' }}>
          {reservations.map(({ inscription, eventName }, index) => (
            <motion.div
              key={inscription.id}
            ...
            >
              ...
              <div style={{ flexGrow: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
                  <h3 style={{ fontSize: '1.2rem', color: 'var(--color-primary)', margin: 0 }}>{eventName}</h3>
                  <span className="badge badge-primary">{inscription.status || 'CONFIRMED'}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', color: '#6B7280', fontSize: '0.9rem' }}>
                  <Calendar size={14} />
                  Reservado el {new Date(inscription.inscriptionDate).toLocaleDateString('es-DO', { year: 'numeric', month: 'long', day: 'numeric' })}
                </div>
              </div>
        ...
        <div style={{ textAlign: 'center', padding: '5rem 2rem', background: 'white', borderRadius: '1.5rem', border: '1px dashed #D1D5DB' }}>
          <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>Tickets</div>
          <h3 style={{ fontSize: '1.5rem', color: 'var(--color-text-dark)', marginBottom: '0.5rem' }}>Aun no tienes reservaciones</h3>
          <p style={{ color: 'var(--color-text-muted)', marginBottom: '2rem' }}>Explora nuestro catalogo y vive el Caribe al maximo.</p>
